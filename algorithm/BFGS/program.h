...
// This is the L-BFGS-B routine implemented in Fortran 77.
	...
	extern"C" {
#endif
	void _stdcall setulb_ (int* n, int* m, double x[], double l[], 
...
public:
  ...
  Program (int n, double* x, double* lb, double* ub, long int* btype,
...
private:
  ...
  long int     iprint;  // The print level.
  long int     maxiter; // The maximum number of iterations.
  double  pgtol;   // Convergence parameter passed to L-BFGS-B.
  double  factr;   // Convergence parameter passed to L-BFGS-B.
  long int     m;       // The number of variable corrections to the 
