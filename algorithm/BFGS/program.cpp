...
// -----------------------------------------------------------------
Program::Program (int n, double* x, double* lb, double* ub, long int* btype, 
...
void Program::callLBFGS (const char* cmd) {
  ...
  int csave_len = strlen(csave);
  setulb_(&n,&m,x,lb,ub,btype,&f,g,&factr,&pgtol,wa,iwa,task,&iprint,
	  csave,lsave,isave,dsave,task_len,csave_len);
}
