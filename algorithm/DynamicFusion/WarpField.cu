...
namespace dfusion
{
	__global__ void warp_mesh_kernel(const  GpuMesh::PointType*__restrict__ vsrc,
		...
		GpuMesh::PointType* vdst, GpuMesh::PointType* ndst, Mat33 R, float3 t, int n)
	{
...
#pragma unroll
		for (int k = 0; k < 8; k++)
		{
			if (i < n)
			{
				vdst[i] = GpuMesh::to_point(R*GpuMesh::from_point(vsrc[i]) + t);
				ndst[i] = GpuMesh::to_point(R*GpuMesh::from_point(nsrc[i]));
			}
			i += blockDim.x;
		}
	}
	void WarpField::warp(GpuMesh& src, GpuMesh& dst)
	{
		...
		warp_mesh_kernel << <grid, block >> >(src.verts(), src.normals(), 
			dst.verts(), dst.normals(), R, t, src.num());
