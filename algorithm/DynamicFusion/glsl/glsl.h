...
* New in RC4:
   ...
   > Added GetProgramObject in class glShader to return the OpenGL Program Object
...
// high level interface to simplify setup of a vertex/fragment shader.
   ...
   public:
       ...
       glShader* loadfromFile(const string& vertexFile, const string& fragmentFile);    //!< load vertex/fragment shader from file. If you specify 0 for one of the shaders, the fixed function pipeline is used for that part. \param vertexFile Vertex Shader File. \param fragmentFile Fragment Shader File.
